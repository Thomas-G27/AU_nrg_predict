...
class Command:
    ...
    ### MODIFIED (added trycatch)
        try :
            cmd_num = int(cmd_num)
        except:
            cmd_num = float(cmd_num)
            cmd_num = int(cmd_num)
    ###
    ...
    ### MODIFIED (added float)
                    feedrate_factor = min(feedrate_factor, float(buf.max_z_feedrate))
    ###
    ...
    ### MODIFIED (added lines)
                    buf.max_z_jerk = float(buf.max_z_jerk)
                    buf.max_e_jerk = float(buf.max_e_jerk)
    ### 
